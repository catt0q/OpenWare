...
public class PaletteHelper implements Helper {
  ...
  /*  public static Color astolfo(boolean clickgui, int yOffset) {
        float speed = clickgui ? ClickGui.speed.getValue() * 100 : HUD.time.getValue() * 100;
   ...
   public static Color astolfo(int yOffset) {
       // HUD global color time removed; keep a fixed speed for compatibility
       float speed = 65f * 100f;
       float hue = (System.currentTimeMillis() % (int) speed) + yOffset;
       if (hue > speed) {
           hue -= speed;
       }
